<template>
	<view class="w">
		...
		<view class="list u-p-20">  
			<view class="list-item card u-p-20 u-p-l-40 u-p-r-40 u-radius-6 bg-white uni-shadow-base u-m-b-20" v-for="item in my_reservation_list" :key="item.id">
		...
		uni.showModal({
			...
			success: async (res) => {
				if (res.confirm) {
					...
					if(res2.code == 1) {
						let i = my_reservation_list.value.map(ele => ele.id).indexOf(item.id)
						my_reservation_list.value[i].confirm = 1
						uni.showToast({
							title: res.msg,
							icon: 'none'
						})  
